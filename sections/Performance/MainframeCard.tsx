...
const MainframeCard: React.FC<MainframeCardProps> = ({
    ...
    return (
        <motion.div
            ...
            viewport={{ once: true }}
            onClick={onClick}
            className={`group relative p-4 sm:p-5 xl:p-6 rounded-2xl lg:rounded-[1.75rem] bg-[#050508] border border-white/5 transition-all duration-500 hover:border-white/10 cursor-pointer overflow-hidden flex flex-col ${className}`}
        >
            ...
            <div className="relative z-10 flex flex-col h-full">
                <div className="flex items-center justify-between mb-3 sm:mb-4 xl:mb-5">
                    <div className="flex items-center gap-2.5 sm:gap-3">
                        <div className={`w-8 h-8 sm:w-9 sm:h-9 xl:w-10 xl:h-10 rounded-xl flex items-center justify-center border bg-white/5 transition-all duration-500 group-hover:scale-110 ${accentColors[color].split(' ')[2]} ${accentColors[color].split(' ')[3]}`}>
                            <Icon size={18} />
                        </div>
                        <div>
                            <h3 className="text-sm sm:text-base font-black text-white uppercase tracking-tighter leading-none">{title}</h3>
                            {isLoading ? (
                                ...
                                subtitle && <p className="text-slate-500 text-[8px] font-mono tracking-widest uppercase mt-0.5">{subtitle}</p>
                            )}
                        </div>
                    </div>
                    <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white/5 flex items-center justify-center text-white/20 group-hover:text-white group-hover:bg-white/10 transition-all">
