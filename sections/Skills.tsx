...
// Internal Component for the Advanced Card
const SkillCard = ({ type }: { type: 'frontend' | 'backend' }) => {
  ...
  return (
    ...
    >
      {/* 1. Dynamic Border Glow (Behind) */}
      <motion.div
        className="absolute inset-0 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-300"
        style={{ background: borderLight }}
      />
      
      {/* 2. Inner Content Container (Relative to maintain flow/height) */}
      <div className="relative h-full bg-[#050508] rounded-[3.4rem] overflow-hidden m-[1px]">
         
         {/* 3. Spotlight Effect on Background */}
         <motion.div
            className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
            style={{ background: spotlight }}
         />

         {/* 4. Tech Grid Pattern Reveal */}
         <div 
